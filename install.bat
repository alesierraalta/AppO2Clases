...
python -c "import sqlite3; conn=sqlite3.connect('gimnasio.db'); cursor=conn.cursor(); print('Verificando columna activo...'); cursor.execute('PRAGMA table_info(horario_clase)'); columns = cursor.fetchall(); column_names = [col[1] for col in columns]; activo_existe = 'activo' in column_names; desactivacion_existe = 'fecha_desactivacion' in column_names; print(f'Columna activo: {\"EXISTE\" if activo_existe else \"NO EXISTE\"}'); print(f'Columna fecha_desactivacion: {\"EXISTE\" if desactivacion_existe else \"NO EXISTE\"}'); if not activo_existe or not desactivacion_existe: print('Agregando columnas faltantes...'); if not activo_existe: cursor.execute('ALTER TABLE horario_clase ADD COLUMN activo BOOLEAN DEFAULT 1'); print('Columna activo agregada.'); if not desactivacion_existe: cursor.execute('ALTER TABLE horario_clase ADD COLUMN fecha_desactivacion DATE'); print('Columna fecha_desactivacion agregada.'); conn.commit(); print('Modificaciones aplicadas correctamente.'); cursor.execute('PRAGMA table_info(horario_clase)'); columns = cursor.fetchall(); print('Esquema final de la tabla:'); for col in columns: print(f'  {col[0]}: {col[1]} ({col[2]})'); conn.close(); exit(0 if 'activo' in [col[1] for col in columns] else 1)"
if %errorlevel% neq 0 (
    echo ADVERTENCIA: La verificacion de la columna 'activo' falló. Intentando método alternativo...
    python -c "import sqlite3; conn=sqlite3.connect('gimnasio.db'); c=conn.cursor(); try: c.execute('ALTER TABLE horario_clase ADD COLUMN activo BOOLEAN DEFAULT 1'); conn.commit(); print('Columna activo agregada.'); except Exception as e: print(f'Error: {e}'); try: c.execute('ALTER TABLE horario_clase ADD COLUMN fecha_desactivacion DATE'); conn.commit(); print('Columna fecha_desactivacion agregada.'); except Exception as e: print(f'Error: {e}'); conn.close()"
    echo Verificación final después del método alternativo...
    python -c "import sqlite3; conn=sqlite3.connect('gimnasio.db'); c=conn.cursor(); c.execute('PRAGMA table_info(horario_clase)'); cols=[col[1] for col in c.fetchall()]; print('Verificación final:'); print('La columna activo ' + ('EXISTE' if 'activo' in cols else 'NO EXISTE') + ' en la base de datos.'); conn.close()"
)
